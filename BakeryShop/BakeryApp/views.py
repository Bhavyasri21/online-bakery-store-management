...
from BakeryApp.forms import ReForm,ItemsForm,UsgForm,Rltype,Rlupd,Pfupd,Changepwd
from BakeryApp.models import Bakery,Bakerylist,User,Rolereq
from django.contrib import messages
from django.contrib.auth.decorators import login_required
from django.core.mail import send_mail
from BakeryShop import settings
...
class Home(View):
	...
	def get(self,request):
		w=Bakery.objects.filter(uid_id=request.user.id)
		t=Bakery.objects.all()
		s=Bakery.objects.all()#category
		g=None #product
		categoryId=request.GET.get('g')
		if categoryId:
			g=Bakerylist.objects.get(categoryId)
		else:
			g=Bakerylist.objects.all()
...
@login_required
def bakerylist(r):
	y=Bakery.objects.filter(uid_id=r.user.id)
	if r.method=="POST":
		...
		if t.is_valid():
			...
			c.uid_id=r.user.id
			c.save()
			messages.success(r,"Bakery Added Successfully")
			return redirect('/rlist')
	t=ReForm()
	return render(r,'app/bakerylist.html',{'q':t,'a':y})

@login_required
def rstup(r,m):
	k=Bakery.objects.get(id=m)
	if r.method=="POST":
		e=ReForm(r.POST,r.FILES,instance=k)
		if e.is_valid():
			e.save()
			messages.warning(r,"{} Bakery Updated Successfully".format(k.rname))
			return redirect('/rlist')
	e=ReForm(instance=k)
...
@login_required
def rstdl(r,s):
	g=Bakery.objects.get(id=s)
	if r.method=="POST":
		messages.info(r,"{} Bakery Deleted Successfully".format(g.rname))
		g.delete()
		return redirect('/rlist')
...
@login_required
def rstvw(r,a):
	s=Bakery.objects.get(id=a)
	return render(r,"app/restview.html",{'z':s})

@login_required
def bakeryitems(re):
	st=list(Bakery.objects.filter(uid_id=re.user.id))
	mm=Bakerylist.objects.all()	
	d,i={},0
	for mp in mm:
		for h in st:
			if mp.rsid_id==h.id:
	...
	u=ItemsForm()
	return render(re,'app/bakeryitems.html',{'q':u,'a':st,'m':d.values()})

@login_required
def itemupt(re,p):
	h=Bakerylist.objects.get(id=p)
	if re.method=="POST":
